...
# Configurações
n = 5 # Numero de horas anteriores para serem analisados
...
def plotarComparacoes(model, x_teste, y_teste, estatisticas=1):
    ...
    plt.xlabel('Quantidade de pontos de movimentação')
    plt.ylabel('Stress')
    plt.legend()
    plt.show()


previsores, stress = loopAnteriores(x, y)
...
rlr = ReduceLROnPlateau(monitor='loss', factor=0.2, patience = 3, verbose=1)
...
model = criarModelo()
model.fit(x = previsores, y = stress, batch_size = 64, epochs=200, 
              callbacks=[es,rlr,tensorboard], validation_data=(X_teste, Y_teste),
              verbose=0)
